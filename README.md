...
Built against a four-phase build brief plus a V2 strategy ruleset (personal
documents distilled from paid courses — kept out of this public repo; the
"§" references below point at their sections).
...
The app self-bootstraps in the cloud: on first boot it downloads the latest
database snapshot from this repo's `data` release (~40 MB), and the
`daily-data` GitHub Actions workflow refreshes that snapshot every night
after EDGAR posts the day's filings. The sidebar's **Catch up filings**
button pulls anything newer on demand.

One-time setup:

1. Go to https://share.streamlit.io → sign in with GitHub → **Create app** →
   pick this repo, branch `master`, main file `dashboard.py`.
2. In this repo's **Actions** tab, enable workflows (first run can be
   triggered manually via "Run workflow" on `daily-data`).

(For a private fork, the app additionally needs a fine-grained PAT with
`Contents: Read` on the repo, set as the Streamlit secret `GITHUB_TOKEN`.)

Free-tier notes: the app sleeps after inactivity and wakes on visit (first
load re-downloads the snapshot); yfinance price fetches run from the cloud
container and cache for the container's lifetime.
