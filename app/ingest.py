...
def catch_up_days(conn) -> list[date]:
    """Days the daily catch-up should (re-)ingest: from the last finalized day
    (so outages heal themselves) through today, minimum a week back."""
    today = date.today()
    last = conn.execute("SELECT MAX(day) FROM ingested_days").fetchone()[0]
    gap = (today - date.fromisoformat(last)).days + 1 if last else 7
    span = max(7, gap)
    return edgar.available_index_days(today - timedelta(days=span), today)
